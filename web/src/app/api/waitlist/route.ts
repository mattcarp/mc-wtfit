...
export async function POST(req: Request) {
  ...
  const r = await sendEmail({
    to: email,
    subject: 'Confirm: you want to know what the fuck that is',
    text: `One click and you're on the WTFIT list:\n\n${link}\n\nIf this wasn't you, ignore this email and nothing happens.\n\nWTFIT. Proceeds from things you let go go to the Gozo SPCA by default.`,
    html: `<div style="font-family:system-ui,sans-serif;max-width:520px;margin:auto;padding:24px;color:#0A0A0B">
      <p style="font:800 28px/1 Arial Narrow,Arial,sans-serif;text-transform:uppercase;margin:0 0 16px">What the fuck is <span style="background:#D4FF3F">this?</span></p>
      <p>One click and you're on the WTFIT list.</p>
      <p><a href="${esc(link)}" style="display:inline-block;background:#0A0A0B;color:#fff;padding:14px 22px;text-decoration:none">Confirm my email</a></p>
